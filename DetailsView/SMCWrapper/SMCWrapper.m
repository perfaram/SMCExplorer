...
//  SMCWrapper.m
//
//  Created by Perceval FARAMAZ on 09/12/2014, based on work by Fergus Morrow and Apple Corp.
...
//
//  Functions prefixed with "SMC" or "_" are private
#import "SMCWrapper.h"

@interface SMCWrapper()
...
-(BOOL) readKey:(NSString *)key asString:(NSString **)str{
    ...
    result = [self SMCReadKey: [key UTF8String]
                  outputValue: &val];
...
-(UInt32) SMCReadIndexCount {
	...
	[self SMCReadKey:"#KEY" outputValue:&val];
...
-(SMCDump) dump {
	...
	returnData.valuesArray = CFDictionaryCreateMutable(NULL, 0, &keyCB, &valCB);
	returnData.typesArray = CFDictionaryCreateMutable(NULL, 0, &keyCB, &valCB);
	NSString* stringKey;
	NSString* valueKey;
	NSString* typeKey;
	...
	for (i = 0; i < totalKeys; i++) {
		...
		result = [self SMCCall:KERNEL_INDEX_SMC forKeyData:&inputStructure outputKeyDataIn:&outputStructure];
		if (result != kIOReturnSuccess)
			continue;
		...
		result = [self SMCReadKey:key outputValue:&val];
		stringKey = [NSString stringWithFormat:@"%s" , key];
		[self getStringRepresentation:val.bytes forSize:val.dataSize ofType:val.dataType inBuffer:cStr[0]];
		valueKey = [[NSString alloc] initWithCString:cStr encoding:NSUTF8StringEncoding];
		typeKey = [NSString stringWithFormat:@"%-4s",val.dataType];
		CFDictionaryAddValue(returnData.valuesArray, (__bridge const void *)(stringKey), (__bridge const void *)(valueKey));
		CFDictionaryAddValue(returnData.typesArray, (__bridge const void *)(stringKey), (__bridge const void *)(typeKey));
		//printVal(val);
	}
	return returnData;
}
...
-(id) init{
    if (self = [super init]){
        if (![self SMCOpen]){
            NSLog(@"Unable to open SMC connection.");
        }
    }
    return self;
}
